...
правки и список реально изменившихся блоков вида "id: было «...» стало
«...»". Скажи, точно ли эти изменения выполняют именно эту правку — не
наоборот, не мимо цели, не пустышка (было эквивалентно стало). Верни ТОЛЬКО
JSON {"ok": true|false, "reason": "..."} без пояснений."""
...
def _check(request, diff):
    diff_text = "\n".join(f'{d["id"]}: было «{d["before"]}» стало «{d["after"]}»' for d in diff)
    messages = [
