...
{"op":"bookmark","id":"p12","name":"ref1"} — поставить закладку на абзац: ЦЕЛЬ для перекрёстной ссылки. Без неё поле REF ссылаться не на что, поэтому нумерованная ссылка делается парой: сначала bookmark на абзац-источник, потом field с instr "REF ref1 \\h" там, где нужна ссылка. Имя — латиницей, без пробелов
...
def _struct_note(b, a):
    ...
    if b.get("bookmarks", 0) != a.get("bookmarks", 0):
        parts.append(f'закладок {b.get("bookmarks", 0)} → {a.get("bookmarks", 0)}')
    if b.get("runs") != a.get("runs"):
        ...
        # Список runs меняется от любой правки текста (куски режутся иначе),
        # поэтому сравнение runs даёт запись и там, где оформление осталось
        # прежним: Проверяющий читал строки вида «оформление «жирным: «Атлас»»
        # → «жирным: «Атлас»»» и «без оформления» → «без оформления» — шум,
        # заявляющий изменение, которого не было (прогон за рулём 2026-07-30).
        if old_fmt != new_fmt:
...
_ID_FIELDS = {
    ...
    "field": ("id",),
    # Закладка адресуется абзацем — цель для перекрёстной ссылки (field REF).
    "bookmark": ("id",),
}
