...
def chat(messages: list[dict]) -> dict:
    ...
    if r.status_code >= 500:
        # 5xx — сбой на стороне провайдера, той же природы, что обрыв
        # транспорта: замер w25 умер целиком на «500 inference error» в
        # середине корпуса, потому что ретрая для этого случая не было, а
        # вызывающий (bench/run.py) ловит только TransportError. Тот же
        # ОДИН ретрай. 4xx не ретраим — это ответ по существу (например
        # переполнение контекста), повтор даст то же самое.
        r = httpx.post(url, **kwargs)
    if r.is_error:
